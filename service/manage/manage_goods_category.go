...
func (m *ManageGoodsCategoryService) AddCategory(req manageReq.MallGoodsCategoryReq) (err error) {
	yanz := manage.MallGoodsCategory{}
	global.GVA_DB.Where("category_level=? AND category_name=? AND is_deleted=0 AND parent_id!=?", req.CategoryLevel, req.CategoryName, req.ParentId).First(&yanz)
	if yanz.CategoryName == req.CategoryName {
...
func (m *ManageGoodsCategoryService) DeleteGoodsCategoriesByIds(ids request.IdsReq) (err error, goodsCategory manage.MallGoodsCategory) {
	err = global.GVA_DB.Where("category_id in ?", ids.Ids).Delete(&manage.MallGoodsCategory{}).Error
	global.GVA_DB.Where("goods_category_id in ?", ids.Ids).Delete(&manage.MallGoodsInfo{})
	return err, goodsCategory
}
