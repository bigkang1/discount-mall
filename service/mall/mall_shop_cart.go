...
func getMallShoppingCartItemVOS(cartItems []mall.MallShoppingCartItem) (err error, cartItemsRes []mallRes.CartItemResponse) {
	...
	for _, cartItem := range cartItems {
		...
		if _, ok := newBeeMallGoodsMap[cartItemRes.GoodsId]; ok {
			...
			var sellingprice float64
			if newBeeMallGoodsTemp.IsDiscount == 1 && int(time.Now().Unix()) < newBeeMallGoodsTemp.DiscountEndTime {
				sellingprice, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", newBeeMallGoodsTemp.SellingPrice*newBeeMallGoodsTemp.OriginalPrice), 64)
			} else {
				sellingprice, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", newBeeMallGoodsTemp.OriginalPrice), 64)
			}
			...
			cartItemRes.SellingPrice = sellingprice
