...
func (m *MallOrderService) PaySuccess(orderNo string, payType int) (err error) {
	...
	if mallOrder != (manage.MallOrder{}) {
		...
		err = global.GVA_DB.Select("currency,is_deleted").Where("user_id=?", mallOrder.UserId).First(&userInfo).Error
		if err != nil || userInfo.IsDeleted != 0 || userInfo.Currency < mallOrder.TotalPrice {
			return errors.New("支付失败,金额不够")
		}

		global.GVA_DB.Model(&manage.MallUser{}).Where("user_id=?", mallOrder.UserId).Update("currency", userInfo.Currency-mallOrder.TotalPrice)
